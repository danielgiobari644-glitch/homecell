...
window.setHofCategory = function(cat) {
  ...
  cats.forEach(c => {
    ...
    if (btn) {
      if (c === cat) {
        btn.className = "px-4 py-2 text-xs font-black rounded-xl transition-all cursor-pointer bg-amber-500 text-white shadow-xs flex items-center gap-1.5";
      } else {
    ...
    return `
      <div class="p-4 rounded-2xl glass-panel flex items-center justify-between gap-4 border ${
        ...
        <div class="flex items-center gap-3 min-w-0">
          <div class="w-8 h-8 rounded-xl ${rank <= 3 ? 'bg-amber-400 text-white font-black' : 'bg-slate-100 dark:bg-zinc-800 text-slate-500'} flex items-center justify-center font-bold text-xs shrink-0">
            #${rank}
          </div>
